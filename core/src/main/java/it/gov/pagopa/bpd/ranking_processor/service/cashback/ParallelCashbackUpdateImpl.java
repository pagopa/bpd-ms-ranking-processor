...
/**
 * Parallel implementation of {@link CashbackUpdateStrategyTemplate}
 */
@Slf4j
@Component
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
class ParallelCashbackUpdateImpl extends CashbackUpdateStrategyTemplate {

    @Autowired
    public ParallelCashbackUpdateImpl(WinningTransactionDao winningTransactionDao,
                                      CitizenRankingDao citizenRankingDao) {
        super(winningTransactionDao, citizenRankingDao);

        if (log.isTraceEnabled()) {
            log.trace("ParallelCashbackUpdateImpl.SerialCashbackUpdateImpl");
        }
    ...
    protected Map<String, CitizenRanking> aggregateData(Stream<CitizenRanking> stream,
        ...
        if (log.isTraceEnabled()) {
            log.trace("ParallelCashbackUpdateImpl.aggregateData");
        }
        if (log.isDebugEnabled()) {
            log.debug("stream = {}, keyMapper = {}, valueMapper = {}, mergeFunction = {}", stream, keyMapper, valueMapper, mergeFunction);
        }

        return stream.parallel().collect(Collectors.toConcurrentMap(keyMapper, valueMapper, mergeFunction));
    }
